// <copyright file="DayNightR1RenderCapture.Debug.cs" company="River-Mochi">
...
// File: Systems/Controls/DayNightR1RenderCapture.Debug.cs
// Purpose: R1 Debug diagnostic. Captures the same Day -> Night frames
// before and after HDRP post-processing without changing the game camera.
...
namespace CityWatchdog.Systems
{
    ...
    internal enum DayNightR1CaptureStage
    {
        BeforePostProcess = 0,
        AfterPostProcess = 1,
    }

    internal sealed class DayNightR1CapturePass : CustomPass
    {
        private readonly DayNightR1CaptureStage m_Stage;

        internal DayNightR1CapturePass(
            DayNightR1CaptureStage stage)
        {
            m_Stage = stage;
        }
        ...
        {
            DayNightR1RenderCapture.TryCapture(
                m_Stage,
                ctx);
        }
    }

    internal static class DayNightR1RenderCapture
    {
        // Six rendered frames cover the E2B X-ray and near-black window.
            ...
            int token)
        {
            ...
            LogUtils.Info(
                $"[CWD-DN-R1] begin token={token} frames={kCaptureFrameCount} width={kCaptureWidth} folder={s_RunFolder}");
        }
            ...
            CustomPassContext ctx)
        {
            ...
            bool[] scheduled =
                stage == DayNightR1CaptureStage.BeforePostProcess
                    ? s_BeforeScheduled
                    : s_AfterScheduled;
            ...
            ctx.cmd.RequestAsyncReadback(
                ...
                request =>
                {
                    ...
                    {
                        LogUtils.WarnOnce(
                            $"day-night-r1-readback-{token}-{captureIndex}-{stageName}",
                            () =>
            ...
            LogUtils.Info(
                $"[CWD-DN-R1] queued token={token} index={captureIndex} unityFrame={unityFrame} stage={stageName} size={width}x{height} sourceFormat={sourceFormat} file={fileName}");
                ...
                s_AfterScheduled[captureIndex])
            {
                ...
                LogUtils.Info(
                    $"[CWD-DN-R1] capture queued token={token} frames={kCaptureFrameCount} folder={s_RunFolder}");
            }
        }
        ...
        private static void EnsureVolumes()
        {
            ...
            if (s_BeforeVolume == null)
            {
                s_BeforeVolume =
                    CreateVolume(
                        "CWD-R1-BeforePostProcess",
                        CustomPassInjectionPoint.BeforePostProcess,
                        DayNightR1CaptureStage.BeforePostProcess,
                        camera);
            }
            else if (camera != null)
            {
            ...
            if (s_AfterVolume == null)
            {
                s_AfterVolume =
                    CreateVolume(
                        "CWD-R1-AfterPostProcess",
                        CustomPassInjectionPoint.AfterPostProcess,
                        DayNightR1CaptureStage.AfterPostProcess,
                        camera);
            }
            else if (camera != null)
            {
        ...
        {
            string info =
                "City Watchdog R1 HDR buffer diagnostic\n" +
                $"Token: {token}\n" +
