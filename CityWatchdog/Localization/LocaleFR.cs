            ...
            Dictionary<string, int> indexCounts)
        {
            ...
            Dictionary<string, string> entries = new Dictionary<string, string>
            {
                ...
                { m_Settings.GetOptionLabelLocaleID(nameof(Setting.UsageText)), "<Affichage>\n" +
                    "1. Bouton [i] : masquer/afficher TOUS les infobulles du jeu — bâtiments, cims, outils et icônes du menu inférieur.\n" +
                    "2. Noms de routes : masquer/afficher les noms des routes. Raccourci : \\.\n" +
                    "3. Noms de districts : masquer/afficher les noms de districts sans changer les limites.\n" +
                    "4. Flèches de route : forcer les flèches de sens unique (masque aussi les noms de routes).\n" +
                    "5. Icône de barre CWD : afficher/masquer les infobulles du panneau City Watchdog.\n" +
                    "\n" +
                    ...
                    "3. Toggle All coupe/réactive tout rapidement, ou ouvrez une section pour changer des icônes précises.\n" +
                    "4. Masque seulement les icônes ; ne corrige pas le problème de la ville.\n" +
                    "\n" +
                    "<Aides d'argent>\n" +
                    "1. Ajouter ou retirer de l'argent : utilisez <Montant du raccourci argent> avec les touches [ et ].\n" +
                    "2. L'ajout automatique ajoute de l'argent quand la ville passe sous la limite choisie.\n" +
                    "3. Convertir une sauvegarde Argent illimité ne sert qu'aux villes créées en Argent illimité et est <irréversible>.\n" +
                    "\n" +
                    "<Infobulles du menu inférieur>\n" +
                    "Money View ajoute les tendances d'argent et de population à la barre, avec plus de détails au survol.\n" +
                    "\n" +
                    "<Jalon personnalisé>\n" +
                    "Réglez l'argent initial et les jalons dans Argent et jalons > RÉGLAGES DE DÉPART DE NOUVELLE VILLE avant de charger ou commencer une ville." },
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.ToggleNotificationsKeyboardBinding)), "<Raccourci> pour la même action que le bouton <[TOGGLE ALL]> en ville.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.ToggleRoadNamesKeyboardBinding)), "<Raccourci> pour masquer ou afficher instantanément les noms de routes du jeu.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.ToggleAllTooltipsKeyboardBinding)), "<Raccourci> pour masquer ou afficher TOUTES les infobulles du jeu — bâtiments, citoyens, outils et icônes du menu du bas.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MoneyViewMode)), "Choisissez si le texte de tendance affiche les valeurs horaires ou mensuelles pour l’argent et la population.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MoneyTooltipMode)), "Choisissez le niveau de détail dans l’infobulle d’argent.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MoneyTooltipFontScale)), "Ajuste la <taille de police> des nombres dans l’infobulle Money View.\n" +
                    "Jeu par défaut = 100%\n" +
                    "<Mod par défaut = 120%>\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.PopulationTooltipFontScale)), "Ajuste la <taille de police> des nombres de population.\n" +
                    "Jeu par défaut = 100%\n" +
                    "<Mod par défaut = 120%>\n" +
                ...
                { m_Settings.GetOptionLabelLocaleID(nameof(Setting.MiniHudEnabled)), "Notifications Mini HUD" },
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MiniHudEnabled)), "Affiche un petit HUD en ville avec les compteurs de notification les plus importants.\n" +
                    "Utilisez-le comme barre d'alerte rapide sans ouvrir le panneau City Watchdog complet." },

                { m_Settings.GetOptionLabelLocaleID(nameof(Setting.MiniHudMode)), "Mode Mini HUD" },
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MiniHudMode)), "Choisissez les lignes de notification utilisées par le Mini HUD.\n" +
                    "Alertes les plus actives affiche les plus grands compteurs actuels.\n" +
                    "Favoris affiche seulement les lignes marquées comme favorites dans le panneau City Watchdog." },
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MiniHudItemCount)), "Choisissez combien d'icônes de notification le Mini HUD peut afficher à la fois." },

                { m_Settings.GetOptionLabelLocaleID(nameof(Setting.MiniHudOrientation)), "Orientation Mini HUD" },
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MiniHudOrientation)), "Choisissez si les icônes du Mini HUD sont en ligne ou en colonne." },
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MiniHudPlacement)), "Choisissez où le Mini HUD apparaît.\n" +
                    "Déplaçable permet de le déplacer dans l'interface de la ville." },
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MiniHudHideZero)), "Quand activé [ ✓ ], le Mini HUD masque les lignes de notification dont le compteur est 0." },

                { m_Settings.GetOptionLabelLocaleID(nameof(Setting.MiniHudGlassStyle)), "Style verre" },
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MiniHudGlassStyle)), "Ajoute un fond doux de type verre derrière le Mini HUD pour une meilleure lisibilité." },
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.InitialMoney)), "Définit l’argent de départ pour une nouvelle ville avec <argent limité>, ou la première ville chargée,\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.CustomMilestone)), "Activez <avant de charger ou créer une ville> pour débloquer immédiatement le milestone choisi après chargement.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MilestoneLevel)), "Choisissez le milestone à débloquer au prochain chargement de ville.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.ManualMoneyAmount)), "Utilisez ce montant avec les raccourcis Ajouter et Retirer de l’argent.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.AddMoneyKeyboardBinding)), "Raccourci pour <Ajouter de l’argent> en ville." },
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.SubtractMoneyKeyboardBinding)), "Raccourci pour <Retirer de l’argent> en ville." },
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.AutomaticAddMoney)), "Quand activé [ ✓ ], City Watchdog vérifie le solde pendant qu’une ville est chargée.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.AutomaticAddMoneyThreshold)), "Si l’ajout automatique est activé et que le solde passe sous cette valeur,\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.WriteNotificationAuditLog)), "<Inutile pour le jeu normal.>\n" +
