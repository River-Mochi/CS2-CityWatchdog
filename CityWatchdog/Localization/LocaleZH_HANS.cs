    ...
    public sealed class LocaleZH_HANS : IDictionarySource
    {
            ...
            Dictionary<string, int> indexCounts)
        {
            ...
            Dictionary<string, string> entries = new Dictionary<string, string>
            {
                ...
                { m_Settings.GetOptionLabelLocaleID(nameof(Setting.PanelButtonsOnlyStart)), "启动时只显示按钮" },
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.PanelButtonsOnlyStart)),
                    "启用 [ ✓ ] 后，从左上角按钮打开 City Watchdog 时会先显示较小的纯按钮视图。\n" +
                    "用标题栏箭头或行数按钮展开完整面板。" },
