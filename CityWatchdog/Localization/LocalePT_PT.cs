...
// Purpose: European Portuguese (pt-PT) for City Watchdog Options UI menu.
    ...
    public sealed class LocalePT_PT : IDictionarySource
    {
            ...
            Dictionary<string, int> indexCounts)
        {
            ...
            Dictionary<string, string> entries = new Dictionary<string, string>
            {
                ...
                { m_Settings.GetOptionLabelLocaleID(nameof(Setting.UsageText)), "<Alternâncias de visualização>\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.ToggleNotificationsKeyboardBinding)), "<Atalho> para a mesma ação do botão <[TOGGLE ALL]> no jogo.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.ToggleRoadNamesKeyboardBinding)), "<Atalho> para ocultar ou mostrar instantaneamente os nomes de ruas vanilla.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.ToggleAllTooltipsKeyboardBinding)), "<Atalho> para ocultar ou mostrar TODOS os tooltips do jogo — prédios, cidadãos, ferramentas e ícones do menu inferior.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MoneyViewMode)), "Escolha se as tendências mostram valores por hora ou por mês para dinheiro e população.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MoneyTooltipMode)), "Escolha quanto detalhe aparece no tooltip de dinheiro.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MoneyTooltipFontScale)), "Ajusta o <tamanho da fonte> dos números do Money View.\n" +
                    "Padrão do jogo = 100%\n" +
                    "<Padrão do mod = 120%>\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.PopulationTooltipFontScale)), "Ajusta o <tamanho da fonte> dos números da população.\n" +
                    "Padrão do jogo = 100%\n" +
                    "<Padrão do mod = 120%>\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MiniHudEnabled)), "Mostra um pequeno HUD na cidade com as contagens de notificação mais importantes.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MiniHudMode)), "Escolha quais linhas de notificação o Mini HUD usa.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MiniHudItemCount)), "Escolha quantos ícones de notificação o Mini HUD pode mostrar de uma vez." },

                { m_Settings.GetOptionLabelLocaleID(nameof(Setting.MiniHudOrientation)), "Orientação Mini HUD" },
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MiniHudOrientation)), "Escolha se os ícones do Mini HUD ficam em linha ou coluna." },
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MiniHudHideZero)), "Quando ativado [ ✓ ], o Mini HUD oculta linhas de notificação com contagem 0." },

                { m_Settings.GetOptionLabelLocaleID(nameof(Setting.MiniHudGlassStyle)), "Estilo vidro" },
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MiniHudGlassStyle)), "Adiciona um fundo suave tipo vidro atrás do Mini HUD para melhorar a leitura." },
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.InitialMoney)), "Define o saldo inicial de uma nova cidade com <dinheiro limitado>, ou da primeira cidade carregada,\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.CustomMilestone)), "Ative <antes de carregar ou iniciar uma cidade> para liberar o milestone escolhido logo após o carregamento.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MilestoneLevel)), "Escolha o milestone para liberar no próximo carregamento da cidade.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.AutomaticAddMoney)), "Quando ativado [ ✓ ], City Watchdog verifica o saldo enquanto uma cidade está carregada.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.AutomaticAddMoneyThreshold)), "Se o dinheiro automático estiver ativado e o saldo cair abaixo deste valor,\n" +
                    "adiciona o valor automático escolhido." },

                { m_Settings.GetOptionLabelLocaleID(nameof(Setting.AutomaticAddMoneyAmount)), "Valor automático de dinheiro" },
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.AutomaticAddMoneyAmount)), "Valor adicionado cada vez que o automático dispara.\n" +
