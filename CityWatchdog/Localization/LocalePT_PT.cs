    ...
    public sealed class LocalePT_PT : IDictionarySource
    {
            ...
            Dictionary<string, int> indexCounts)
        {
            ...
            Dictionary<string, string> entries = new Dictionary<string, string>
            {
                ...
                // --- Money View ---
                { m_Settings.GetOptionLabelLocaleID(nameof(Setting.MoneyView)), "Mostrar detalhes ao passar o rato" },
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MoneyView)),
                    "Mostra valores numéricos de tendência junto às setas vanilla de dinheiro e população na barra inferior.\n" +
                    "É uma visualização leve ao passar o rato na barra <apenas visualização>;\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MoneyViewMode)),
                    ...
                    "Mensal usa receitas do orçamento menos despesas para dinheiro, e uma projeção de 24 horas para população." },
                { m_Settings.GetOptionLocaleID("MoneyViewModeHourly"), "Por hora (/h)" },
                { m_Settings.GetOptionLocaleID("MoneyViewModeMonthly"), "Mensal (/mo)" },

                { m_Settings.GetOptionLabelLocaleID(nameof(Setting.MoneyTooltipMode)), "Estilo da tooltip de dinheiro" },
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MoneyTooltipMode)),
                    "Escolha quantos detalhes aparecem na tooltip de dinheiro ao passar o rato.\n" +
                    "Compacto = predefinido na primeira instalação.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.MoneyTooltipFontScale)),
                    "Ajusta o <tamanho da fonte> dos números da tooltip do Money View.\n" +
                    "Predefinição do jogo = 100%\n" +
                    "<Predefinição do mod = 120%>\n" +
                    "Passe o rato sobre Dinheiro na parte inferior do ecrã.\n" +
                    "Pedido por jogadores que têm dificuldade em ver tooltips pequenas no jogo." },

                { m_Settings.GetOptionLabelLocaleID(nameof(Setting.PopulationTooltipFontScale)), "Tamanho da fonte da população" },
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.PopulationTooltipFontScale)),
                    "Ajusta o <tamanho da fonte> dos números da tooltip de população.\n" +
                    "Predefinição do jogo = 100%\n" +
                    "<Predefinição do mod = 120%>\n" +
                    "Passe o rato sobre População na parte inferior do ecrã." },
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.ManualMoneyAmount)),
                    ...
                    "Isto não faz nada a menos que use o atalho para adicionar/subtrair dinheiro na cidade.\n" +
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.AutomaticAddMoney)),
                    ...
                    "- Recomendo usar dinheiro manual com atalho (<[> ou <]>) quando precisar  em vez desta opção automática, mas ela está aqui se a quiser usar." },
                ...
                { m_Settings.GetOptionLabelLocaleID(nameof(Setting.ToggleAllTooltipsKeyboardBinding)), "Desativar todos os tooltips do mouse" },
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.ToggleAllTooltipsKeyboardBinding)),
                    "<Atalho> para ocultar ou mostrar instantaneamente TODOS os tooltips do jogo ao passar o rato — edifícios, cims, ferramentas e ícones do menu inferior.\n" +
                    "<Os popups próprios de dinheiro/população do City Watchdog permanecem ativos>; são controlados pela opção Money View acima.\n" +
                    "Mesmo efeito de clicar no ícone [i] do painel City Watchdog na cidade." },
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.ConfirmUnlimitedMoneySaveConversion)),
                    "<Faça primeiro uma cópia de segurança da cidade>.\n" +
                    "Converte uma cidade iniciada com Dinheiro ilimitado para uma cidade normal com desafios de dinheiro regulares.\n" +
                    "Ativar isto liberta o botão <[Converter save de Dinheiro ilimitado]> quando a cidade carregada é do tipo <Dinheiro ilimitado>.\n" +
                    "O City Watchdog não pode desfazer esta conversão.\n" +
                    "Se tem cidades normais, não se preocupe; isto não é necessário." },
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.ConvertUnlimitedMoneySave)),
                    ...
                    "O botão fica <desativado/cinzento> a menos que a cidade carregada seja do tipo <Dinheiro ilimitado>\n" +
                    "e <Conversor de dinheiro ilimitado> esteja ON [ ✓ ].\n" +
                    "Faça um save de cópia de segurança e use por sua conta e risco; o City Watchdog não pode desfazer esta conversão." },

                { m_Settings.GetOptionWarningLocaleID(nameof(Setting.ConvertUnlimitedMoneySave)),
                    "Converter esta cidade de Dinheiro ilimitado para dinheiro limitado normal?\n" +
                    "Guarde uma cópia de segurança PRIMEIRO; o City Watchdog não pode desfazer isto.\n" +
                    "Tem a certeza?" },
                ...
                { m_Settings.GetOptionDescLocaleID(nameof(Setting.OpenLog)),
                    ...
                    "Se o ficheiro de log não existir, abre a pasta Logs/." },
                ...
                { m_Settings.GetOptionLabelLocaleID(nameof(Setting.UsageText)),
                    ...
                    "1. Botão [i]: oculta/mostra TODAS as tooltips do jogo ao passar o rato (edifícios, cims, ferramentas, ícones do menu inferior).\n" +
                    "2. Botão de nomes das ruas: oculta/mostra rótulos de nomes das ruas. Atalho: \\.\n" +
                    "3. Botão de setas das ruas: força setas de sentido único ligadas/desligadas (também oculta nomes das ruas).\n" +
                    "4. Ícone da barra de título do CWD: mostra/oculta tooltips do painel City Watchdog.\n" +
                    "\n" +
                    "<Alertas de notificação>\n" +
                    "1. Clique no botão City Watchdog (canto superior esquerdo), ou prima Shift+N, para abrir o painel.\n" +
                    "2. Botão de ordenação para ascendente/descendente.\n" +
                    "3. Toggle All para tudo Off/On rapidamente, ou expanda uma secção para mudar ícones específicos.\n" +
                    "4. Só mostra ou oculta ícones; não corrige o problema da cidade.\n" +
                    "\n" +
                    "<Ajuda com dinheiro>\n" +
                    "1. Adicionar ou subtrair dinheiro: use as teclas predefinidas do <Valor do atalho de dinheiro> [ e ].\n" +
                    ...
                    "Money View adiciona valores de tendência à barra de dinheiro e população e detalhes extra ao passar o rato.\n" +
                    "\n" +
                    "<Marco personalizado>\n" +
